...
具体要求、完整模板和角色目录内的文件布局见[本职 Skill](../skills/sprite-designer-workflow/SKILL.md)。

上游资料仓库、版本和文件，与本次产物的保存仓库分别选择，可以不同；读取资料不限制其所在目录。
沿用已明确的仓库，只问影响当前工作的缺项；暂时不需要的可稍后配置。
本角色新增或更新的业务产物统一保存在业务仓库的 `docs/designer/`，功能说明、决定、报告和本角色加工的必要附件都放在该角色目录内。其他角色产物分别位于各自 `docs/<角色名>/`，交接保持真实归属。角色和 Skill 安装目录只放能力资源，正常代码与测试保留原项目位置。
