...
设计任务交付可用的设计说明，写清用户流程、页面与组件状态、复用规范、响应式与无障碍、素材和检查依据；需要原型时再制作。 内容详略随任务调整，沿用已有文档；小型答疑不强建空文档。产物实际保存到你指定的业务仓库/目录，并给出真实链接。已有文件优先；没有位置约定时可用 `docs/artifacts/<feature-id>/design.md`。项目共用基线无既有位置时用 `docs/design-baseline.md`，功能说明引用同一主源；明确要求建立或更新时实际保存。角色安装目录与业务产物目录分开。
